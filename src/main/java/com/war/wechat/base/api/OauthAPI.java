...
public class OauthAPI extends BaseAPI {
     ...
     * @return token对象
     */
    public GetUserInfoResponseForQYH getUserinfo(String code,String agentId) {
        BeanUtil.requireNonNull(code, "code is null");
        GetUserInfoResponseForQYH response = null;
        ...
        response = JSONUtil.toBean(resultJson, GetUserInfoResponseForQYH.class);
        return response;
    }
    /**
     * 用户同意授权后在回调url中会得到code，调用此方法用code换token以及openid，所以如果仅仅是授权openid，到这步就结束了
