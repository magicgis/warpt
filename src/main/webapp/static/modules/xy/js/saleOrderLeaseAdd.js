...
(function($) {
		...
		customPicker.show(function(items) {
			if (items[0].value) {
				...
				var pramMap = {
					...
					mui.post(mypath + '/xy/saleOrder/findTypeList', pramMap, function(
							response) {
						if (response) {
	...
	var sendUsernameInput = document.getElementById('sendUsername');
	var senduserIdInput = document.getElementById('senduserId');
	var driverNameInpt = document.getElementById('driverName');
	var driverIdInpt = document.getElementById('driverId');	
	sendUsernameInput.addEventListener('tap', function(event) {
		vipPicker.show(function(items) {
			if (items[0].value) {
				sendUsernameInput.value = items[0].text;
				senduserIdInput.value = items[0].value;
			} else {
				return false;
			}
		});
