...
function loadOrder(isNew, paramMap) {
	...
	mui.post(mypath + '/xy/difference/findLeaseOrder', paramMap, function(response) {
		// var html = [];
		pageNo++; // 加载完毕翻页增加1
		...
		for (var i = 0; i < dataList.length; i++) {
			...
			li.id = dataList[i].id;
			var statusName = dataList[i].status == 1 ? '已处理' : '未处理';
...
//					+ '" class="ui_edit mui-btn mui-btn-yellow mui-icon mui-icon-compose" btn_type="edit" ></a>';
			...
			appendStr += '<div class="mui-media-body">商品名称：'
					+ dataList[i].productName
					+ '<p class="mui-ellipsis" style="color: #000000;font-size:12px;">发货单号：'
					+ dataList[i].saleNo
					+ '</p>'
					+ '<p class="mui-ellipsis" style="color: #000000;font-size:12px;">状态：'
					+ statusName + '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;发货日期：'
					+ dataList[i].businData 
					+ '</p>'
					+ '<p class="mui-ellipsis" style="color: #000000;font-size:12px;">差异数量：'
					+ dataList[i].diffNum
					+ '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;客户：'
					+ dataList[i].customName					
					+ '</p>'					
					+ '<p class="mui-ellipsis" style="color: #000000;font-size:12px;">备注：'
					+ dataList[i].remarks  
					+ '</p>';
			appendStr += '</div></a>';
...
//	window.location.href = mypath + "/xy/difference/edit?difference.id=" + id;
};
