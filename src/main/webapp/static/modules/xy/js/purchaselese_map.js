...
(function(window, undefined) {
	var selectEditProMap = {
		...
		saveOrder : function(paramId, isSuccess) {
			// 差异值存在则获取
			selectEditProMap.setRemarksVal();
			// 选择商品构造
			var itemList = [];
			for (key in selectEditProMap.selectProductMap) {
				...
				if (setMap.num > 0) {
					...
					itemList.push({
						...
						saleNum : setMap.saleNum,
						purchaseNum : setMap.num,
						diffNum : setMap.diff
					});
				}
			}
			var receivingId = document.getElementById("receivingId").value;
			...
			mui.post(mypath + '/xy/purchaseOrder/saveOrderLease', pramMap, function(
					response) {
				if (response.success) {
					window.location.href = mypath + "/xy/purchaseOrder/listLeaseApp";
				} else {
