...
(function($) {
	...
	// 新增按钮
	$('body').on('tap', '#addBtn', function(event) {
		window.location.href = mypath + "/xy/saleOrder/add";
	});
})(mui);
...
function loadOrder(isNew, paramMap) {
	...
	mui.post(mypath + '/xy/saleOrder/findOrderLease', paramMap, function(response) {
		// var html = [];
		pageNo++; // 加载完毕翻页增加1
...
//						+ '" class="ui_edit mui-btn mui-btn-yellow mui-icon mui-icon-compose" btn_type="edit" ></a>';
				...
				// appendStr += '<a href="javascript:void(0)" data-id="'
				// + dataList[i].id
				// + '" class="ui_del mui-btn mui-btn-red mui-icon mui-icon-trash"
				// btn_type="del" onclick=""></a>';
				// }
				appendStr += '</div>';
...
function editClick(id) {
	window.location.href = mypath + "/xy/saleOrder/leaseEdit?id=" + id;
};
function deletClick(id) {
	mui.confirm('确认删除该条记录？', '操作确认', ['确认', '取消'], function(e) {
		if (e.index == 0) {
			mui.post(mypath + '/xy/saleOrder/deleteOrder', {
				paramId : id
			}, function(response) {
				if (response.success) {
					pageNo = 1;
