...
(function($) {
	...
	$('body').on('tap', '#addBtn', function(event) {
		window.location.href = mypath + "/xy/purchaseOrder/add";
	});
})(mui);
...
function loadOrder(isNew, paramMap) {
	...
	mui.post(mypath + '/xy/purchaseOrder/findOrder', paramMap, function(
			response) {
		// var html = [];
		pageNo++; // 加载完毕翻页增加1
...
//						+ '" class="ui_edit mui-btn mui-btn-yellow mui-icon mui-icon-compose" btn_type="edit" ></a>';
				...
				appendStr += '<div class="mui-media-body">订单号：'
						+ dataList[i].orderNo
						+ '<p class="mui-ellipsis" style="color: #000000;font-size:12px;">收货总数：'
...
function editClick(id) {
	window.location.href = mypath + "/xy/purchaseOrder/edit?id=" + id;
};
function deletClick(id) {
	mui.confirm('确认删除该条记录？', '操作确认', ['确认', '取消'], function(e) {
		if (e.index == 0) {
			mui.post(mypath + '/xy/purchaseOrder/deleteOrder', {
				paramId : id
			}, function(response) {
				if (response.success) {
					pageNo = 1;
